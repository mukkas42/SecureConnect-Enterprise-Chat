-- Fix Voice Calls Column Name in Delete Function
-- Use recipient_id instead of callee_id
...
BEGIN
    ...
    -- 2. Delete attachments for messages sent by user (via message_id relationship)
    DELETE FROM attachments WHERE message_id IN (
        SELECT id FROM messages WHERE sender_id = target_user_id
    );
    GET DIAGNOSTICS attachments_count = ROW_COUNT;
    
    -- 3. Delete encrypted files created by user (uses created_by column)
    DELETE FROM encrypted_files WHERE created_by = (
        SELECT auth_user_id FROM users WHERE id = target_user_id
    );
    GET DIAGNOSTICS encrypted_files_count = ROW_COUNT;
    
    -- 4. Delete messages sent by user (encrypted content)
    ...
    -- 12. Delete voice calls where user is caller or recipient (FIXED: recipient_id not callee_id)
    DELETE FROM voice_calls WHERE caller_id = target_user_id OR recipient_id = target_user_id;
...
-- Add comment
COMMENT ON FUNCTION delete_encrypted_data_only(UUID, UUID) IS 'Deletes encrypted data only. Fixed: voice_calls uses recipient_id not callee_id.';
