...
-- The issue is that 'conversation_id' is ambiguous in the participant count subquery

-- Drop and recreate get_user_conversations_enhanced function with explicit table aliases
DROP FUNCTION IF EXISTS get_user_conversations_enhanced(uuid) CASCADE;
...
BEGIN
    ...
    SELECT 
        ...
        -- Fix: Use explicit table alias to avoid ambiguity
        (SELECT COUNT(*) FROM conversation_participants cp_count WHERE cp_count.conversation_id = c.id) as participant_count,
