...
-- This migration updates the format from secureconnect_ to secureconnect (no underscore)
...
BEGIN
    ...
    RETURN 'secureconnect' || result; -- Prefix with secureconnect (no underscore)
END;
$$ LANGUAGE plpgsql;

-- Update existing unique identifiers to remove underscore
-- Convert secureconnect_ prefixed identifiers to secureconnect prefixed (remove underscore)
UPDATE users 
SET unique_identifier = 'secureconnect' || substr(unique_identifier, 11)
WHERE unique_identifier LIKE 'secureconnect_%' AND length(unique_identifier) = 18;

-- Also update any remaining QC format identifiers to new format
UPDATE users 
SET unique_identifier = 'secureconnect' || substr(unique_identifier, 3)
...
BEGIN
    FOR user_record IN SELECT id FROM users WHERE unique_identifier IS NULL OR length(unique_identifier) != 17 LOOP
        ...
        SET unique_identifier = (
            SELECT 'secureconnect' || string_agg(
