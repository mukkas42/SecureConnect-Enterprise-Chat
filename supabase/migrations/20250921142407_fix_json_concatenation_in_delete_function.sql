-- Fix JSON Concatenation in Delete Function
-- Replace JSON concatenation with proper JSON building
...
COMMENT ON FUNCTION delete_encrypted_data_only(UUID, UUID) IS 'Deletes only encrypted data (messages, files, keys) while preserving user account and profile. Fixed JSON building without concatenation.';
