-- Fix Note to Self Function Migration
-- This migration fixes the get_user_conversations_enhanced function that was broken 
-- due to referencing non-existent 'content' column in messages table
...
COMMENT ON FUNCTION get_user_conversations_enhanced(UUID) IS 'Fixed function that properly handles encrypted_content column and prioritizes note-to-self conversations';

-- Log fix completion
DO $$
BEGIN
    RAISE NOTICE '=== NOTE TO SELF FUNCTION FIX COMPLETED ===';
    RAISE NOTICE 'Fixed get_user_conversations_enhanced function to work with encrypted_content column';
    RAISE NOTICE 'Note-to-self conversations will now load properly';
END $$;
