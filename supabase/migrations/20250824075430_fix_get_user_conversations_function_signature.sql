-- Fix Function Signature Issue for get_user_conversations
-- Drops the existing function completely before recreating to avoid signature conflicts

-- Drop both functions completely to avoid signature conflicts
DROP FUNCTION IF EXISTS get_user_conversations(UUID);
DROP FUNCTION IF EXISTS get_user_conversations_enhanced(UUID);

-- Recreate the enhanced function with properly qualified column references
...
-- Create the simpler get_user_conversations function that the WebSocket handler calls
