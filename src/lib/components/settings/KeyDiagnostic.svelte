<script>
	...
	async function runKeyDiagnostic() {
		...
		try {
			...
			results.push(`🔍 [KEYS] SecureConnect_pq_keypair exists: ${!!oldKeys}`);
			...
			if (mlKemKeys && oldKeys) {
				try {
					...
					results.push(`🔍 [FORMAT] SecureConnect_pq_keypair has publicKey: ${!!oldParsed.publicKey}`);
					results.push(`🔍 [FORMAT] SecureConnect_pq_keypair has privateKey: ${!!oldParsed.privateKey}`);
			...
			if (!mlKemKeys) {
				...
				results.push('⚠️ [WARNING] Missing SecureConnect_pq_keypair (old format)');
