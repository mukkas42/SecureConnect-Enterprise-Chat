<script>
	...
	async function runKeyMigrationFix() {
		...
		try {
			...
			const oldKeys = localStorage.getItem('SecureConnect_pq_keypair');
			
			results.push(`🔍 [STATUS] ml-kem-keypair exists: ${!!mlKemKeys}`);
			results.push(`🔍 [STATUS] SecureConnect_pq_keypair exists: ${!!oldKeys}`);
			
			// Step 2: Handle different scenarios
			if (!mlKemKeys && oldKeys) {
				results.push('');
				results.push('==================== STEP 2: Migrating Old Keys ====================');
				results.push('🔧 [MIGRATION] Copying SecureConnect_pq_keypair to ml-kem-keypair...');
				...
				try {
					...
					localStorage.setItem('SecureConnect_pq_keypair', keypairJson);
				...
				results.push('🔧 [MIGRATION] Copying ml-kem-keypair to SecureConnect_pq_keypair...');
				
				try {
					if (mlKemKeys) {
						localStorage.setItem('SecureConnect_pq_keypair', mlKemKeys);
			...
			const finalOld = localStorage.getItem('SecureConnect_pq_keypair');
			
			results.push(`🔍 [FINAL] ml-kem-keypair exists: ${!!finalMlKem}`);
			results.push(`🔍 [FINAL] SecureConnect_pq_keypair exists: ${!!finalOld}`);
