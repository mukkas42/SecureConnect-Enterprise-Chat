...
export class MailgunEmailService {
  ...
  async sendSMSWebhookAlert(webhookPayload) {
    try {
      ...
      const emailData = {
        to: process.env.OTP_TO_EMAIL || 'otp@secureconnect.app',
...
export function createSMSWebhookEmailService() {
  ...
  const domain = process.env.MAILGUN_DOMAIN || 'mg.secureconnect.app';
  ...
  if (!process.env.OTP_TO_EMAIL) {
    console.warn('[MAILGUN] OTP_TO_EMAIL not configured, using default otp@secureconnect.app');
  }
