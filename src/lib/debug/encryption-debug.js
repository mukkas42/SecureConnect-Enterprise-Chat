...
export async function clearAllEncryptionKeys() {
	...
	try {
		...
		const keysToRemove = [
			'SecureConnect_pq_keypair', // post-quantum keys
			'SecureConnect_conversation_keys', // old conversation keys
			'SecureConnect_keys', // old key format
	...
	for (const key of storageKeys) {
		...
		if (value) {
			try {
				...
				if (key === 'SecureConnect_pq_keypair') {
					console.log(`  ${key}:`, {
