...
const PREFIX = 'SecureConnect Enterprise Chat';
const IDENTIFIER_LENGTH = 17; // SecureConnect Enterprise Chat + 8 characters
const BODY_LENGTH = 8;

/**
 * Generate a new unique identifier
 * @returns {string} A unique identifier in format SecureConnect Enterprise Chat + 8 characters
 */
...
export function parseUniqueIdentifier(userInput) {
    ...
    // Handle case where it starts with SecureConnect Enterprise Chat (case insensitive)
    const prefixMatch = cleaned.match(/^SecureConnect Enterprise Chat/i);
    if (prefixMatch) {
        const body = cleaned.slice(prefixMatch[0].length).toUpperCase();
        return PREFIX + body;
    }
    
    // If it doesn't start with SecureConnect Enterprise Chat, return cleaned and normalized
