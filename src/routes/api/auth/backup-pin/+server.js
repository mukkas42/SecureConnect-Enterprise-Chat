...
export async function POST({ request }) {
	try {
		...
		// Update the Supabase Auth password to the PIN to allow direct login
		const { error: passwordError } = await getServiceRoleClient().auth.admin.updateUserById(
			user.id,
			{ password: pin }
		);
		
		if (passwordError) {
			console.error('Critical: Failed to set auth password:', passwordError);
			return json({ error: 'Failed to sync PIN with authentication system. ' + passwordError.message }, { status: 500 });
		}
