...
export async function POST(event) {
	...
	try {
		...
		// Development/Mock Mode: Skip real SMS if credentials are placeholders or if explicitly enabled
		const isMockEnabled = publicEnv.VITE_ENABLE_MOCK_SMS === 'true' || 
							(dev && (!env.TWILIO_ACCOUNT_SID || env.TWILIO_ACCOUNT_SID.includes('your_twilio')));

		if (isMockEnabled) {
			logger.info('MOCK SMS MODE ENABLED - Bypassing real SMS sending');
			
			// Log the "sent" code to server console for the developer
			const mockCode = '123456'; // Standard mock code
			console.log('\n--- [MOCK SMS] ---');
			...
			return json({
				success: true,
				message: 'MOCK: Verification code sent successfully (check server console)',
				isMock: true,
