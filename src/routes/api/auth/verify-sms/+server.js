...
export async function POST(event) {
	...
	try {
		...
		if (useSession && authHeader) {
			...
			// OTP Verification Flow (with Mock Support)
			const isMockEnabled =
				publicEnv.VITE_ENABLE_MOCK_SMS === 'true' ||
				(dev &&
					(!env.TWILIO_ACCOUNT_SID ||
						env.TWILIO_ACCOUNT_SID.includes('your_twilio')));

			if (isMockEnabled && verificationCode === '123456') {
				logger.info('MOCK SMS VERIFICATION - Bypassing real OTP check');

				// Create service role client for user management
				const serviceSupabase = createClient(PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
					auth: { autoRefreshToken: false, persistSession: false }
				});

				// 1. Ensure user exists in Auth and has a temporary password
				const tempPassword = 'MockPassword123!';
				let authUser = null;
				
				try {
					const normalizedSearchPhone = phoneNumber.replace(/^\+/, '');
					logger.info('Looking up mock user via listUsers', { phone: phoneNumber, normalized: normalizedSearchPhone });
					const { data, error: listError } = await serviceSupabase.auth.admin.listUsers({ perPage: 1000 });
					if (!listError && data?.users) {
						authUser = data.users.find(u => {
							if (!u.phone) return false;
							const dbPhone = u.phone.replace(/^\+/, '');
							return dbPhone === normalizedSearchPhone;
						});
					}
				} catch (e) {
					logger.error('Failed to look up user in mock flow', { error: e });
				}
				...
				// 2. Sign in with the temporary password to get a real session
				...
				if (sessionError) {
					logger.error('Mock sign-in failed', { error: sessionError });
					throw sessionError;
				}

				verifyData.session = sessionData.session;
				verifyError = null;
			} else {
				// Original OTP verification flow
		...
		// Log to scratch file for the assistant to see
		try {
			const logPath = 'd:/HARSHITHA_PROJECTS/AI/SecureConnect Enterprise Chat-web/scratch/verify-error.log';
			const logData = `[${new Date().toISOString()}] ${error instanceof Error ? error.message : 'Unknown'}\n${error instanceof Error ? error.stack : ''}\n\n`;
