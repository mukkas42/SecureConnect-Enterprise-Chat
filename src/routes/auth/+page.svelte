<script>
	...
	async function setBackupPin() {
		...
		if (backupPin.length < 6 || backupPin.length > 12) {
			messages.error('PIN must be 6-12 digits');
			return;
		}
		...
		try {
			...
			messages.success('Backup PIN set and keys backed up! Welcome to SecureConnect Enterprise Chat!');
			backupPin = '';
			confirmBackupPin = '';
			goto('/chat');
		} catch (error) {
...
<svelte:head>
	<title>{$t('auth.title')} - SecureConnect Enterprise Chat</title>
</svelte:head>
	...
	<div class="auth-card">
		...
		{#if step === 'verify'}
			<div class="auth-step">
				...
				<form onsubmit={(e) => { e.preventDefault(); verifySMS(); }}>
					<div class="input-group">
		...
		{#if step === 'profile'}
			<div class="auth-step">
				...
				<form onsubmit={(e) => { e.preventDefault(); completeProfile(); }}>
		...
		{#if step === 'backup'}
			<div class="auth-step">
				...
				<form onsubmit={(e) => { e.preventDefault(); setBackupPin(); }}>
					<div class="input-group">
						<label for="backup-pin">Backup PIN (6-12 digits) *</label>
						<div class="password-input">
							<input
								...
								maxlength="12"
								oninput={(e) => { backupPin = e.target.value.replace(/\D/g, '').slice(0, 12); }}
					...
					<div class="input-group">
						...
						<div class="password-input">
							<input
								...
								maxlength="12"
								oninput={(e) => { confirmBackupPin = e.target.value.replace(/\D/g, '').slice(0, 12); }}
					...
					<div class="input-group">
						...
						<div class="password-input">
							<input
								...
								maxlength="12"
								oninput={(e) => { restorePin = e.target.value.replace(/\D/g, '').slice(0, 12); }}
								placeholder="Enter your 6-12 digit PIN"
								required
